...
def generate_invoice_pdf(vente: dict, details: list) -> str:
    ...
    coord_parts = []
    if company_addr: coord_parts.append(company_addr)
    if company_tel:  coord_parts.append(f"Tel : {company_tel}")
    if company_mail: coord_parts.append(f"Email : {company_mail}")
    ...
    details_client = []
    if client_adr: details_client.append(client_adr)
    if client_vil: details_client.append(client_vil)
    if client_tel: details_client.append(f"Tel : {client_tel}")
    if client_mail: details_client.append(f"Email : {client_mail}")
    ...
    for i, (header, cx) in enumerate(zip(headers, col_x)):
        if i == 0:
            ...
            c.drawCentredString(
                cx + col_widths[i] * (W - 2*margin) / 2, y + 0.25*cm, header)
    ...
    def draw_total_row(cy, label, value, bold=False, color=DARK, bg=None):
        if bg:
            c.setFillColor(bg)
            c.rect(totaux_x, cy - 0.1*cm, totaux_w, 0.65*cm, fill=1, stroke=0)
    ...
    c.roundRect(totaux_x, y - 1.5*cm, totaux_w, 0.7*cm, 3, fill=1, stroke=0)
    draw_total_row(y - 1.45*cm, "Reste à payer", f"{reste:.2f}", bold=True, color=RED)
    ...
    c.drawCentredString(
        W/2, 0.45*cm,
        f"Facture generee le {datetime.now().strftime('%d/%m/%Y a %H:%M')} — {company_name}"
    )
