...
class SaleModel:
    ...
    @staticmethod
    def get_all(search=None, statut=None) -> list:
        """Retourne toutes les ventes avec filtres optionnels."""
        conditions, params = [], []
        if search:
            pattern = f"%{search}%"
            conditions.append(
                "(v.numero_facture LIKE %s OR c.nom LIKE %s OR c.prenom LIKE %s)")
    ...
    def get_by_id(sale_id: int) -> dict | None:
        """Retourne une vente par son ID avec toutes les infos."""
    ...
    def get_kpi() -> dict:
        ...
        result = db.execute_query("""
            SELECT
                COALESCE(SUM(CASE WHEN DATE(date_vente)=CURDATE()
                    AND statut!='annulee' THEN montant_total ELSE 0 END), 0)
                    AS ca_jour,
                COALESCE(SUM(CASE WHEN YEARWEEK(date_vente)=YEARWEEK(CURDATE())
                    AND statut!='annulee' THEN montant_total ELSE 0 END), 0)
                    AS ca_semaine,
                ...
                COUNT(CASE WHEN statut='partielle' THEN 1 END) AS nb_partielles,
                COALESCE(SUM(CASE WHEN statut IN ('en_cours','partielle')
    ...
    def generate_numero() -> str:
        ...
        result = db.execute_query(
            "SELECT COUNT(*) AS total FROM ventes WHERE DATE(date_vente) = CURDATE()",
            fetch_one=True)
        seq = result['total'] + 1
    ...
    def create(sale_data: dict) -> int:
        ...
        Accepte un dictionnaire avec les cles :
        ...
        for ligne in sale_data['panier']:
            db.execute_update("""
                ...
                VALUES (%s, %s, %s, %s)
            """, (sale_id, ligne['produit_id'],
                  ligne['quantite'], ligne['prix_unitaire']))
            # Mettre a jour le stock
            db.execute_update(
                "UPDATE produits SET stock_actuel = stock_actuel - %s WHERE id = %s",
                (ligne['quantite'], ligne['produit_id']))
        return sale_id

    @staticmethod
    def add_payment(sale_id: int, montant: float) -> bool:
        """Ajoute un paiement et met a jour le statut."""
        ...
        if nouveau_paye > total:
            raise ValueError(
                f"Montant depasse le total. Reste : {float(vente['montant_reste']):.2f}")
    ...
    def validate(sale_id: int, montant_reste: float) -> bool:
        """Force le statut a 'payee', en soldant le reste si necessaire."""
        if montant_reste > 0:
            SaleModel.add_payment(sale_id, montant_reste)
        else:
            db.execute_update(
                "UPDATE ventes SET statut='payee' WHERE id=%s", (sale_id,))
        return True
    ...
    def cancel(sale_id: int) -> bool:
        ...
        db.execute_update(
            "UPDATE ventes SET statut='annulee' WHERE id=%s", (sale_id,))
        return True
    ...
    def count() -> int:
        ...
        result = db.execute_query(
            "SELECT COUNT(*) AS total FROM ventes", fetch_one=True)
        return result['total'] if result else 0

    @staticmethod
    def get_recent(limit: int = 8) -> list:
        """Retourne les N dernieres ventes pour le dashboard."""
        return db.execute_query("""
    ...
    def get_dashboard_stats() -> dict:
        ...
        result = db.execute_query("""
            SELECT
                ...
                COUNT(CASE WHEN MONTH(date_vente)=MONTH(CURDATE())
                    AND YEAR(date_vente)=YEAR(CURDATE())
    ...
    @staticmethod
    def get_report_ventes(debut: str, fin: str,
                          vendeur_id: int = None) -> list:
        """Retourne les ventes par periode pour le rapport."""
        params = [debut, fin]
        v_filter = ""
        if vendeur_id:
    ...
    @staticmethod
    def get_report_impayes(debut: str, fin: str,
                           vendeur_id: int = None) -> list:
        """Retourne les impayes par periode pour le rapport."""
        params = [debut, fin]
        v_filter = ""
        if vendeur_id:
            v_filter = "AND v.user_id = %s"
    ...
    def get_report_vendeurs(debut: str, fin: str) -> list:
        """Retourne les stats par vendeur pour le rapport."""
        return db.execute_query("""
